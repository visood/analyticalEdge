...
dummyXYsubmit.test <- as.data.frame(do.call("cbind", (lapply(compDataSubmit[, svord], function(x)makeDummy(x, as.logi=FALSE)))))
...
What variables?
```{r}
acc.cv <- t(sapply(1:10, function(i) analyseVariablesWithCrossValidation(dummyXY, dummyNames[ findIndices(c(1:6, 8:12, 14, 15, 20, 21))], auc=TRUE, retfun=identity)))
mean(apply(acc.cv, 1, mean))
mean(apply(acc.cv, 1, sd))
acc.cv.6 <- t(sapply(1:100, function(i) analyseVariablesWithCrossValidation(dummyXY, dummyNames[ findIndices(c(1:6, 7:10, 12:15))], auc=TRUE, retfun=identity)))
mean(apply(acc.cv.6, 1, mean))
mean(apply(acc.cv.6, 1, sd))

acc.cv.7 <- t(sapply(1:100, function(i) analyseVariablesWithCrossValidation(dummyXY, dummyNames[ findIndices(c(1:15))], auc=TRUE, retfun=identity)))
mean(apply(acc.cv.7, 1, mean))
mean(apply(acc.cv.7, 1, sd))

acc.cv.8 <- t(sapply(1:100, function(i) analyseVariablesWithCrossValidation(dummyXY, dummyNames[ findIndices(c(1:6, 8:12, 14:15))], auc=TRUE, retfun=identity)))
mean(apply(acc.cv.8, 1, mean))
mean(apply(acc.cv.8, 1, sd))


acc.cv.9 <- t(sapply(1:100, function(i) analyseVariablesWithCrossValidation(dummyXY, dummyNames[ findIndices(c(1:6, 8:12, 14, 15, 20, 21))], auc=TRUE, retfun=identity)))
mean(apply(acc.cv.9, 1, mean))
mean(apply(acc.cv.9, 1, sd))

acc.cv.10 <- t(sapply(1:100, function(i) analyseVariablesWithCrossValidation(dummyXY, dummyNames[ findIndices(c(1:6, 8:12, 14, 15, 20, 23))], auc=TRUE, retfun=identity)))
mean(apply(acc.cv.10, 1, mean))
mean(apply(acc.cv.10, 1, sd))

acc.cv.11 <- t(sapply(1:100, function(i) analyseVariablesWithCrossValidation(dummyXY, dummyNames[ findIndices(c(1:6, 8:12, 14, 15, 20, 25))], auc=TRUE, retfun=identity)))
mean(apply(acc.cv.11, 1, mean))
mean(apply(acc.cv.11, 1, sd))

acc.cv.12 <- t(sapply(1:100, function(i) analyseVariablesWithCrossValidation(dummyXY, dummyNames[ findIndices(c(1:6, 8:15,  25))], auc=TRUE, retfun=identity)))
mean(apply(acc.cv.12, 1, mean))
mean(apply(acc.cv.12, 1, sd))

acc.cv.13 <- t(sapply(1:100, function(i) analyseVariablesWithCrossValidation(dummyXY, dummyNames[ findIndices(c(1:6, 8:12, 14:15, 23, 25))], auc=TRUE, retfun=identity)))
mean(apply(acc.cv.13, 1, mean))
mean(apply(acc.cv.13, 1, sd))

acc.cv.14 <- t(sapply(1:100, function(i) analyseVariablesWithCrossValidation(dummyXY, dummyNames[ findIndices(c(1:15, 25))], auc=TRUE, retfun=identity)))
mean(apply(acc.cv.14, 1, mean))
mean(apply(acc.cv.14, 1, sd))

acc.cv.15 <- t(sapply(1:100, function(i) analyseVariablesWithCrossValidation(dummyXY, dummyNames[ findIndices(c(1:15, 25, 37))], auc=TRUE, retfun=identity)))
mean(apply(acc.cv.15, 1, mean))
mean(apply(acc.cv.15, 1, sd))

acc.cv.16 <- t(sapply(1:100, function(i) analyseVariablesWithCrossValidation(dummyXY, dummyNames[ findIndices(c(1:6, 8:15, 25, 37))], auc=TRUE, retfun=identity)))
mean(apply(acc.cv.16, 1, mean))
mean(apply(acc.cv.16, 1, sd))
```
...
auc.fwd <- append(auc.fwd, lapply(modelsConsidered[22:31], function(model) t(sapply(1:10, function(r) analyseVariablesWithCrossValidation(dummyXY, dummyNames[findIndices(c(model))], auc=TRUE, retfun=identity)))))
...
lasso.pred <- predict(lasso.mod, type="response", s=bestlam, newx=X.test)
performance(prediction(lasso.pred, Y.test), "auc")@y.values[[1]]

cofs <- predict(lasso.mod, type="coefficients", s=bestlam)
...
#lasso model, on all vars
X <- as.matrix(dummyXY[,-311])
Y <- dummyXY$Y
lasso.mod <- glmnet(X, Y, family="binomial", alpha=1, lambda=grid)
lasso.pred <- predict(lasso.mod, type="response", s=bestlam, newx=X.test)
performance(prediction(lasso.pred, Y.test), "auc")@y.values[[1]]
lasso.pred.sub <- predict(lasso.mod, type="response", s=bestlam, newx=subX)
...
trynewvarinlasso <- function(i){
  mvars <- c(model[1:30], i)
  X <- as.matrix(dummyXY[, dummyNames[findIndices(mvars)]])
  X <- as.matrix(dummyXY[, -311])
  Y <- dummyXY$Y
  X.train <- subset(X, split==TRUE)
  ...
  plot(cv.out)
  bestlam <- cv.out$lambda.min
  ridge.mod <- glmnet(X.train, Y.train, family="binomial", alpha=1, lambda=grid)
  ridge.pred <- predict(ridge.mod, type="response",s=bestlam, newx=X.test)
  acc <- performance(prediction(ridge.pred, Y.test), "auc")@y.values[[1]]
  
  cofs <- predict(ridge.mod, type="coefficients", s=bestlam)
  nzDNs <- rownames(cofs)[cofs[,1] != 0]
  lassoVars <- (1:102)[sapply(1:102, function(i) length(intersect(dummyNames[findIndices(i)], nzDNs)) > 0 )]
  list(acc=acc, badVars=setdiff(mvars, lassoVars))
}

lassoCV <- function(X, Y, bestlam, withLam=FALSE, retfun=identity, auc=TRUE){
  N <- nrow(X)
  rindexShuffled <- sample(1:N, N, replace=FALSE)
  cvistop <- (1:4)*floor(N/5)
  cvistart <- c(1, cvistop+1)
...
#      cv.out <- cv.glmnet(X.train, Y.train, alpha=1)
      ...
      lasso.pred <- predict(lasso.model, s=bestlam, newx=X.test, type="response")
      if(withLam){
        if(!auc) list(lam=bestlam, acc=tabacc( table(Y.test, lasso.pred > 0.5)))
        else list(lam=bestlam, auc=performance(prediction(lasso.pred, Y.test), "auc")@y.values[[1]])
      }
      else{
        if(!auc) acc=tabacc( table(Y.test, lasso.pred > 0.5))
        else auc=performance(prediction(lasso.pred, Y.test), "auc")@y.values[[1]]
      }
    })
  )  
}